...
**Solution:** _CSV stands for 'comma-separated values'. It is a popular format because the data is in a minimal and basic form that makes it easy to work with when analysed in different softwares in different ways._ 
<br>
<br>
...
read_csv("/Users/nic/Library/CloudStorage/OneDrive-NationalUniversityofSingapore/NM2207/Week 2/playlist_data.csv") 
...
playlist_data <- read_csv("/Users/nic/Library/CloudStorage/OneDrive-NationalUniversityofSingapore/NM2207/Week 2/playlist_data.csv") 
...
```
<br>
```{r, out.height= "400px",out.width= "800px",echo=TRUE,eval=TRUE,fig.cap="Information"}
knitr::include_graphics("/Users/nic/Library/CloudStorage/OneDrive-NationalUniversityofSingapore/NM2207/Week 2/Screenshot 2023-08-21 at 6.12.25 PM.png")
```
<br>

**Question 1.6:** What does the `skip` argument in the read_csv() function do?

**Solution:** _It instructs the program to to skip or ignore a specified number of lines before beginning to read the data._ 
<br>
<br>
...
ggplot(data=playlist_data, mapping=aes(x=Age,y=Rating)) + labs(x="Age (years)", y="Ratings (out of 5)")
...
ggplot(data=playlist_data, mapping=aes(x=Age,y=Rating)) + geom_point() + labs(x="Age (years)", y="Ratings (out of 5)")
...
ggplot(data=playlist_data, mapping=aes(x=Age,y=Rating)) + geom_point(shape=8) + labs(x="Age (years)", y="Ratings (out of 5)")
...
ggplot(data=playlist_data, mapping=aes(x=Age,y=Rating)) + geom_point(shape=8) + labs(title="Relationship between DJs' ages and their ratings", x="Age (years)", y="Ratings (out of 5)", caption = "There is a weakly positive relationship between DJs' ages and their ratings.")
